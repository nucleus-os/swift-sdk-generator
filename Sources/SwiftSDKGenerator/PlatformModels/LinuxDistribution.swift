...
public enum LinuxDistribution: Hashable, Sendable {
    ...
    var release: String {
        switch self {
        case .rhel(let rhel): return rhel.rawValue
        case .ubuntu(let ubuntu): return ubuntu.rawValue
        case .debian(let debian): return debian.rawValue
        }
    }

    var swiftDockerImageSuffix: String {
        switch self {
        case .rhel(let rhel): return "rhel-\(rhel.rawValue)"
        case .ubuntu(let ubuntu): return ubuntu.rawValue
        case .debian(let debian): return debian.rawValue
        }
    }
